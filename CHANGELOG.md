...
- **Action Menu**：修正浮出面板在矩陣格子內被壓成單列高的 bug（見踩坑）。
...
- **`flex-direction:column` 的高瘦卡片放進 `.mcell`（grid 內的 flex-row 格子）會被壓成單列高、內容溢出**（實測 action-menu 面板 `offsetHeight=32` 但 `scrollHeight=151`）。`align-self`／`flex:none`／改父層 `align-items` 都無效，只有撐 `min-height` 有效。修法：scoped `.mcell .am-panel{min-height:max-content;}`（只作用矩陣、不碰 Live 的絕對定位面板）。凡「矩陣格子裡放 flex-column 卡片」的元件都要留意。
